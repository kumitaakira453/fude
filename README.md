...
画像は**文書と同じところの `images/`**（設定で名前を変えられる）へ取り込まれ、本文には
`![](./images/…)` として入る。入れ方は 4 つで、どれも同じ置き場所・同じ名前の付け方を通る。

- 編集面で **`⌘V`** — 撮った画面をそのまま貼る
- 編集面へ **ファイルを落とす** — Finder からドラッグ&ドロップ
- **`/image`** — 枠が出る。ファイルを選ぶ・そこへ落とす・道筋を打つ（絶対でも `./` でも）
- 画像だけの行の**つまみのメニュー** — 「画像をコピー」と「置換」

名前は元のファイル名を残し、重なったときだけ後ろに連番を付ける。貼り付けのように
名前を持たないものは日時から作る。置換しても**古いファイルは消さない**（他の文書から
参照されているかは、その操作の中では分からない）。
...
そのうえで、**今それが何行目にあるか**を出す。控えた範囲・逐語の探索・基準版からの
行差分による移送・近くの近似一致を、費用の安い順に試す。どの段で決まったかによって
`確実` / `候補3` / `推定 0.78` / `不明` の札が付き、**推測を確定と言わない**。
読み手はその行を直接読めばよく、引用文を grep で探し直さなくて済む。
...
| `fude review backfill` | 既にある指摘へ位置の手掛かりを遡って埋める（`--dry-run` で件数だけ）。一度きりの手入れ |
...
件数が多いときは `--brief` で 1 件 1 行に落として絞る（`#ID / 状態 / 行範囲 / 確信度 /
節 / 抜粋` の並び）。`--exit-code` を付けると該当 0 件で終了コード 2 を返す
（エラーの 1 とは区別する）。
